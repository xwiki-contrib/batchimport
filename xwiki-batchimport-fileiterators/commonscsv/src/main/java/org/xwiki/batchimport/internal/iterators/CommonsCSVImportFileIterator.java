 ...
 * @version $Id$
 */
@Component("commonscsv")
@InstantiationStrategy(ComponentInstantiationStrategy.PER_LOOKUP)
public class CommonsCSVImportFileIterator implements ImportFileIterator
{
    @Inject
    ...
    public void resetFile(BatchImportConfiguration config) throws IOException
    {
        ...
        this.csvparser = new CSVParser(sourceInputStream,
            CSVFormat.RFC4180.withDelimiter(config.getCsvSeparator()).withQuote(config.getCsvTextDelimiter()));
        this.csvParserIterator = this.csvparser.iterator();
        logger.debug("Starting to read file " + config.getAttachmentReference() + " as commonscsv");
    }
    ...
    {
        if (this.csvParserIterator == null) {
            return null;
        }
        if (this.csvParserIterator.hasNext()) {
            CSVRecord nextRecord = this.csvParserIterator.next();
            logger.debug("Reading record " + nextRecord.getRecordNumber() + ": " + nextRecord);
            List<String> nextLineAsList = new ArrayList<String>();
            for (String s : nextRecord) {
                nextLineAsList.add(s);
            }
            return nextLineAsList;
        } else {
            return null;
        }
    }
     ...
     * {@inheritDoc}
     * 
     * @see org.xwiki.batchimport.ImportFileIterator#close()
     */
    @Override
